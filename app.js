(() => {
  ...
  const PLAYER_COLORS = {
    Dziadek: "#ff8c2a", Wujek: "#f1c40f", Ojciec: "#1f4aa8", Matka: "#2ecc71",
    Synek: "#9b59b6", Gruby: "#e74c3c", Kowal: "#5bc0ff", Telemans: "#111111",
  };
  ...
  const renderYearTable = (stats) => {
    ...
    const body = MONTHS.map((m, mi) => {
      const row = years.map((y) => {
        ...
        const c = { ...cells[mi] };
        const maxGames = Math.max(...cells.map((x) => x.games));
        const bestWr = Math.max(...cells.map((x) => (x.winrate ?? -1)));
        const cg = [c.games === maxGames && c.games > 0 ? "peak-games" : "", c.games > 15 ? "hot-games" : ""].join(" ").trim();
        const cw = c.winrate !== null && c.winrate === bestWr ? "peak-winrate" : "";
  ...
  const renderCharts = (stats) => {
    ...
    if (dsGames.length) {
      charts.push(new Chart(byId("monthlyMatchesByYearChart"), {
        type: "line",
        data: { labels: MONTHS, datasets: dsGames },
        options: { plugins: { legend: { position: "bottom" } }, spanGaps: false, scales: { y: { beginAtZero: true } } },
      }));
    }
    ...
    if (dsWr.length) {
      ...
      charts.push(new Chart(byId("monthlyWinrateByYearChart"), {
        type: "line",
        data: { labels: MONTHS, datasets: dsWr },
        options: { plugins: { legend: { position: "bottom" } }, spanGaps: false, scales: { y: { min: minWr, max: maxWr } } },
      }));
    }
  };
  ...
  const renderRecent = (matches) => {
    byId("recentMatches").innerHTML = matches.slice(0, 10).map((m) => `
      <article class="match-tile ${m.result}">
        <div class="match-map" style="background-image:${mapImg(m.map)}"></div>
        <div class="match-content">
          <div class="chips-row"><span class="match-result-chip ${m.result}">${m.result.toUpperCase()}</span><span class="side-chip">${m.startingSide || "-"}</span></div>
  ...
  const renderPlayersCumulativeChart = (rows) => {
    const dated = rows.filter((r) => r.date instanceof Date && !Number.isNaN(r.date.valueOf()));
    if (!dated.length) return;

    const uniqueByPlayer = new Map(TEAM_ORDER.map((p) => [p, new Set()]));
    const dailyMap = new Map();

    dated
      .sort((a, b) => a.date.valueOf() - b.date.valueOf())
      .forEach((r, idx) => {
        const p = PLAYER_BY_STEAM[r.steamId] || r.player;
        if (!uniqueByPlayer.has(p)) return;
        const dayKey = r.date.toISOString().slice(0, 10);
        const matchKey = r.id || `${dayKey}_${r.map}_${r.score}_${idx}`;
        const seen = uniqueByPlayer.get(p);
        if (seen.has(matchKey)) return;
        seen.add(matchKey);
        if (!dailyMap.has(dayKey)) dailyMap.set(dayKey, Object.fromEntries(TEAM_ORDER.map((n) => [n, 0])));
        dailyMap.get(dayKey)[p] += 1;
      });

    const labels = Array.from(dailyMap.keys()).sort();
    if (!labels.length) return;

    const running = Object.fromEntries(TEAM_ORDER.map((p) => [p, 0]));
    const datasets = TEAM_ORDER.map((p) => ({
      label: p,
      data: labels.map((d) => {
        running[p] += dailyMap.get(d)?.[p] || 0;
        return running[p];
      }),
      borderColor: PLAYER_COLORS[p] || "#ccc",
      backgroundColor: PLAYER_COLORS[p] || "#ccc",
      borderWidth: 3,
      fill: false,
      tension: 0.2,
      pointRadius: 0,
    ...
    charts.push(new Chart(byId("playersMatchesCumulativeChart"), {
      type: "line",
  ...
  const renderRanking = (rows) => {
    ...
    byId("topRankingTable").querySelector("tbody").innerHTML = rank.map((r) => `<tr><td><div class="player-cell"><img src="${avatar(r.p)}" alt="${r.p}"><span>${r.p}</span></div></td>${cell(r.k, r.k === bk)}${cell(r.a, r.a === ba)}${cell(r.d, r.d === bd)}${cell(r.adr, r.adr === br, 1)}</tr>`).join("");
  };
  ...
  const aggregateMaps = (matches) => {
    ...
    const sumKeys = ["1v5", "1v4", "1v3", "1v2", "1v1", "5K", "4K", "3K", "EBT", "FA", "UD"];
  ...
  const renderTop3MapCharts = (maps) => {
    ...
    charts.push(new Chart(byId("topMapsCountChart"), { type: "bar", data: { labels: top.map((m) => m.map), datasets: [{ data: top.map((m) => m.games), backgroundColor: ["#5f86ff", "#44c978", "#ffbf4d"] }] }, options: { plugins: { legend: { display: false } } } }));
    ...
    charts.push(new Chart(byId("topMapsWinrateChart"), { type: "bar", data: { labels: top.map((m) => m.map), datasets: [{ data: top.map((m) => m.winPct), backgroundColor: ["#5f86ff", "#44c978", "#ffbf4d"] }] }, options: { plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, max: topWrMax } } } }));
  };
  ...
  const render = (rows, matches) => {
    ...
    renderPlayersCumulativeChart(rows);
    renderRanking(rows);
